...
class ReachTransferPillars(base.Task):
  ...
  def intialize_pillars_random(self, physics):
    ...
    # pillar_spawn_radius = 0.85 * physics.named.model.geom_size['floor', 0]
    # Avoid range
    pill2quad = 1.2
    pill2pill = 0.6
    ...
    while penetrating:
      ...
      # print("targ",dist(targ_pos, quad_pos), quad2targ)
      ...
      for i in range(self._n_pillars):
        ...
        if dist(pillar_pos[i], quad_pos) < pill2quad:
          penetrating=True
          continue
        # print(dist(pillar_pos[i], quad_pos))
        # combinatorial
        for j in range(i):
          if dist(pillar_pos[i], pillar_pos[j]) < pill2pill:
            penetrating = True
            continue
          # print(i,j,dist(pillar_pos[i], pillar_pos[j]))
      # print(penetrating)
