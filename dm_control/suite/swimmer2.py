...
def swimmermove(time_limit=_DEFAULT_TIME_LIMIT, random=None,
             environment_kwargs=None):
  """Returns a 6-link swimmer."""
  physics =  _make_swimmer_physics(6)

  task = SwimmerMove(random=random, desired_speed=_MED_SPEED)
...
@SUITE.add('benchmarking')
def swimmertarget(time_limit=_DEFAULT_TIME_LIMIT, random=None,
             environment_kwargs=None):
  """Returns a 6-link swimmer."""
  physics =  _make_swimmer_physics(6)

  task = SwimmerTarget(random=random)
  environment_kwargs = environment_kwargs or {}
  return control.Environment(
      physics, task, time_limit=time_limit, control_timestep=_CONTROL_TIMESTEP,
      **environment_kwargs)
...
def _make_swimmer_physics(n_joints):
...
class Physics(mujoco.Physics):
  ...
  def head_velocity(self):
    """Returns the velocity of the head, in the local frame."""
    return self.named.data.sensordata['head_vel']

  def joints(self):
    """Returns all internal joint angles (excluding root joints)."""
    return self.data.qpos[3:].copy()

class SwimmerBase(base.Task):
  ...
  def __init__(self, random=None):
    ...
    """
    super(SwimmerBase, self).__init__(random=random)
  ...
  def initialize_episode(self, physics):
    ...
    super(SwimmerBase, self).initialize_episode(physics)
...
class SwimmerMove(SwimmerBase):
  """A swimmer `Task` to reach the target or just swim."""

  def __init__(self, random=None, desired_speed=_MED_SPEED):
    """Initializes an instance of `Swimmer`.
    ...
    Args:
      random: Optional, either a `numpy.random.RandomState` instance, an
        ...
        automatically (default).
    """
    super(SwimmerMove, self).__init__(random=random)
    self._desired_speed = desired_speed

  def get_reward(self, physics):
    """Returns a smooth reward."""
    move_reward = rewards.tolerance(
        physics.head_velocity()[0],
...
class SwimmerTarget(SwimmerBase):
  """A swimmer `Task` to reach the target or just swim."""
  ...
  def __init__(self, random=None):
    ...
    Args:
      random: Optional, either a `numpy.random.RandomState` instance, an
        ...
        automatically (default).
    """
    super(SwimmerTarget, self).__init__(random=random)
